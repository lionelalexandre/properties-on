...
import lib_dm
from lib_dm import mm, mm_inner, mm_outer
...
def _solve_dm10_uncoupled(nmrobj, mo_coeff = None, mo_occ = None, mo_energy = None,
    ...
    e_i, e_a = mo_energy[occidx], mo_energy[viridx]
    C_i, C_a = mo_coeff[:, occidx], mo_coeff[:, viridx]
    cart, M = s1.shape[0], s1.shape[1]
    
    #t0_con = time.perf_counter()
    
    C_i, C_a = tf.convert_to_tensor(C_i, dtype=tf.float32), tf.convert_to_tensor(C_a, dtype=tf.float32)
    s1, h1 = tf.convert_to_tensor(s1, dtype=tf.float32), tf.convert_to_tensor(h1, dtype=tf.float32)  
    
    #t1_con = time.perf_counter()
    #dt_con = t1_con-t0_con
    #print("time spent converting to tensor=", dt_con)    
    
    d0 = mm(C_i, tf.transpose(C_i, conjugate=True), method = 'tf')
    dsd = - mm( mm(d0,s1, method = 'tf'), d0, method= 'tf')
    D_ia = tf.Variable(tf.zeros((cart, M, M), dtype=tf.float32))
    ...
    for n in range(cart):
        for i in range(num_occ):
            for a in range(num_vir):
                ...
                x_ia = mm_outer(C_i[:,i], C_a[:,a], method='tf') 
                #x_ia_numpy = np.outer(C_i[:,i], C_a[:,a])
                #frobenius_diff = np.linalg.norm(x_ia - x_ia_numpy)
                #print("Frobenius difference x_ia:", frobenius_diff)

                denominator = e_i[i] - e_a[a]         
            
                A = C_i[:, i]
                B = mm(((h1[n]-e_i[i]*s1[n])/denominator), C_a[:,a], method='tf')
                #B_numpy = np.matmul(((h1[n]-e_i[i]*s1[n])/denominator), C_a[:,a])
                #frobenius_diff = np.linalg.norm(B - B_numpy)
                #print("Frobenius difference AB:", frobenius_diff)
                
                scalar = mm_inner(A, B, method ='tf')  
                #scalar_numpy = np.inner(A, B)
                #frobenius_diff = np.linalg.norm(scalar - scalar_numpy)
                #print("Frobenius difference AB:", frobenius_diff)
                
                
                #t00 = time.perf_counter()
                
                #print('D_ia before assign', D_ia[n])
                update = D_ia[n] + (x_ia * scalar)
                D_ia[n].assign(update)
                #print('D_ia after assign', D_ia[n])
                #D_ia = tf.tensor_scatter_nd_add(D_ia, indices=[[n]], updates=[x_ia * scalar])
    ...
    print("time spent in the loop=", dt)    
    #print("time spent in the update=", dt0)   
                
    D_ai = - tf.transpose(D_ia, perm=[0,2,1])
    D_ii = .5*dsd
    D1 = dsd + D_ia + D_ai
    
    #t0_con_back = time.perf_counter()
    
    D1, D_ii, D_ia, D_ai = D1.numpy(), D_ii.numpy(), D_ia.numpy(), D_ai.numpy()
    
    #t1_con_back = time.perf_counter()
    #dt_con_back = t1_con_back-t0_con_back
    #print("time spent converting back to array=", dt_con_back)    
    
    #t10 = time.perf_counter()
    #dt0 = t10-t00
    #print("time spent in the routine=", dt0)    
    
    return D1, D_ii, D_ia, D_ai
...
def gen_vind(mf, mo_coeff, mo_occ):
    ...
    def vind(mo1):
        ...
        print('mo1.reshape(-1,nmo,nocc).shape', mo1.reshape(-1,nmo,nocc).shape)
        ...
        print('vresp(dm1).shape', vresp(dm1).shape)        #shape is (pert, nmo, nmo) 
        print('v1mo.shape', v1mo.shape, 'v1mo.ravel(),shape', v1mo.ravel().shape)                    #shape is (pert, nmo, nocc) 
        return v1mo.ravel()
    return vind
...
# gauge_orig=None will call GIAO. A coordinate array leads to common gauge
    ...
    def shielding(self, mo1=None, use_dm10=None):
        ...
        if use_dm10 is True:
            ...
            print("At least one argument needs to be different than False")
            sys.exit() 
        else:
            msc_para, para_vir, para_occ = self.para()
...
print('mo10:')
msc_para_m10, para_vir_m10, para_occ_m10 = para(nmr)
#print('dm10:')
#msc_para, para_vir, para_occ = para(nmr, dm10=True)


#vind = gen_vind_ao(mf, mo_coeff, mo_occ)
