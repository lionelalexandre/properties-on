...
def _solve_dm10_uncoupled(nmrobj, mo_energy = None, mo_coeff = None, mo_occ = None, 
    ...
    d0 = np.matmul(C_i, C_i.conj().T)
    dsd = - np.matmul(np.matmul(d0,s1),d0)
    ...
    for n in range(cart):
        for i in range(num_occ):
            for a in range(num_vir):
                ...
                #if np.abs(denominator) < 1e-32:
                    #warnings.warn(f"Small denominator encountered: {denominator:.2e}", RuntimeWarning)
                alpha_ia = np.inner(C_i[:,i],np.matmul((h1[n]-e_i[i]*s1[n])/(denominator),C_a[:,a]))
...
# gauge_orig=None will call GIAO. A coordinate array leads to common gauge
    ...
    def shielding(self, mo1=None, use_dm10=False):
        ...
        if (mo1 is None and not use_dm10):
            self.mo10, self.mo_e10 = self.solve_mo1()
            mo1 = self.mo10
                                      #mo10=mo1 or dm10=True ??? !
...
#msc_para_m, para_vir_m, para_occ_m = para(nmr, mo10=True) #doing mo10, checked
#msc_para, para_vir, para_occ = para(nmr, dm10=None)       #doing mo10, checked
#msc_para, para_vir, para_occ = para(nmr, mo10=None)       #doing mo10, checked 

#print("Comparing msc_para_m with msc_para_dm (expected perturbed density matrix)...")
#test_D1 = msc_para_m - msc_para
#print(np.linalg.norm(test_D1.reshape(-1, test_D1.shape[-1]), ord='fro'), "should be close to zero...")


#print("Comparing para_vir_m with para_vir_dm (expected perturbed density matrix)...")
#test_D1 = para_vir_m - para_vir
#print(np.linalg.norm(test_D1.reshape(-1, test_D1.shape[-1]), ord='fro'), "should be close to zero...")

#print("Comparing para_occ_m with para_occ_dm (expected perturbed density matrix)...")
#test_D1 = para_occ_m - para_occ
#print(np.linalg.norm(test_D1.reshape(-1, test_D1.shape[-1]), ord='fro'), "should be close to zero...")
